...
export function ProfileView({ data, completionPercentage, onEdit, onSubmit, submitting }: ProfileViewProps) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#091F4D] via-[#0E2F6C] to-[#164596] p-8 shadow-xl border border-[#164596]/30 text-white">
        ...
        <div className="relative flex flex-col md:flex-row items-center md:items-start gap-8 z-10">
          ...
          {/* Progress Widget */}
          <div className="flex flex-col items-center justify-center rounded-2xl bg-white/5 border border-white/10 px-5 py-4 w-full md:w-auto text-center backdrop-blur-md">
      ...
      {data.status === 'draft' && (
        <button
          onClick={onSubmit}
          disabled={submitting}
          className="w-full rounded-2xl bg-gradient-to-br from-amber-500 to-amber-600 py-3.5 text-xs font-bold text-white hover:from-amber-600 hover:to-amber-700 transition-all flex items-center justify-center gap-2 shadow-md shadow-amber-500/20 disabled:opacity-50"
        >
      ...
      <button
        onClick={onEdit}
        className="w-full rounded-2xl border-2 border-dashed border-[#0B2C6B]/20 bg-[#0B2C6B]/5 py-3.5 text-xs font-bold text-[#0B2C6B] hover:border-[#0B2C6B]/40 hover:bg-[#0B2C6B]/10 transition-all flex items-center justify-center gap-2 group shadow-sm shadow-[#0B2C6B]/5"
      >
            ...
            <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm space-y-4">
              ...
              <div className="flex items-center gap-3">
                <div className={`h-3 w-3 rounded-full animate-pulse ${
                  av.status === 'open' ? 'bg-emerald-500' :
                  av.status === 'busy' ? 'bg-amber-500' : 'bg-rose-500'
                }`} />
                <span className="text-sm font-bold text-slate-800">
                  {av.status === 'open' ? 'Terbuka untuk Kolaborasi' :
                   av.status === 'busy' ? 'Terbatas / Sibuk' : 'Tidak Tersedia'}
                </span>
              </div>
          ...
          {cvDoc && (
            <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm space-y-3">
              ...
              <div className="flex items-center gap-3 bg-slate-50 border border-slate-100 p-3.5 rounded-xl">
                <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#0B2C6B]/10 text-[#0B2C6B]">
                  <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                  </svg>
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-xs font-bold text-slate-900 truncate" title={cvDoc.file_name}>{cvDoc.file_name}</p>
            ...
            <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                {data.skills.map((skill) => (
                  <span key={skill.id} className="rounded-xl border border-slate-150 bg-slate-50/50 px-3 py-2 text-xs font-semibold text-slate-700 hover:border-slate-350 hover:bg-slate-50 transition-colors">
                    {skill.skill_name}
                    {skill.proficiency && (
                      <span className="ml-1 text-[10px] font-medium text-slate-400 capitalize">
                        ({skill.proficiency})
                      </span>
                    )}
                  </span>
            ...
            <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                {data.languages.map((lang) => (
                  <span key={lang.id} className="rounded-xl border border-slate-150 bg-slate-50/50 px-3 py-2 text-xs font-semibold text-slate-700">
                    {lang.language}
                    <span className="ml-1.5 text-[10px] font-bold text-[#0B2C6B] uppercase tracking-wider">
                      {lang.proficiency}
                    </span>
                  </span>
                ))}
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {cert.credential_url && (
                        <a
                          ...
                          className="mt-2.5 inline-flex items-center gap-1 text-[11px] font-bold text-[#0B2C6B] hover:underline"
                        >
                          Lihat Kredensial
                ...
                {data.portfolios.map((port) => (
                  <div key={port.id} className="rounded-xl border border-slate-200 bg-white p-4 hover:shadow-sm transition-shadow flex flex-col justify-between">
                    <div>
                      ...
                      {port.description && (
                        <p className="text-xs text-slate-500 mt-1.5 leading-relaxed line-clamp-2">{port.description}</p>
                      )}
                    </div>
                    {port.link_url && (
      ...
      {!p?.full_name && data.experiences.length === 0 && data.skills.length === 0 && (
        <div className="rounded-2xl border-2 border-dashed border-slate-200 p-12 text-center bg-white shadow-sm">
